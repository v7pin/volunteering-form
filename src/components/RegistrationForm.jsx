...
function RegistrationForm() {
  ...
  const [formData, setFormData] = useState({
    formType: "volunteering", // Ensure formType is part of the formData
    name: "",
    email: "",
    phone: "",
    whatsapp: "",
    ...
    googleClassroomLink: "",
    whatsappGroupLink: "",
    knowsGoogleClassroom: "",
    understandUnpaid: "",
    joinedGoogleClassroom: "",
    joinedWhatsAppGroup: "",
  });
      ...
      .then((response) => {
        if (response.data) {
          setFormData((formData) => ({
            ...formData,
            googleClassroomLink: response.data.googleClassroomLink || "",
            whatsappGroupLink: response.data.whatsappGroupLink || "",
          }));
        }
      })
      .catch((error) => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-cyan-300 via-cyan-100 to-cyan-50">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md mt-16 mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <RadioButtonQuestion
            question="Do you know how to operate Google Classroom as a student?"
            options={["Yes", "No", "I am ready to learn!"]}
            name="knowsGoogleClassroom"
            selectedOption={formData.knowsGoogleClassroom}
            onChange={handleChange}
          />
          <div className="space-y-3">
            <p className="text-gray-700 font-radio font-medium ">
              Please join the Google Classroom where you will get all the
              updates related to tasks and internship:
            </p>
            <a
              href={formData.googleClassroomLink}
              target="_blank"
              rel="noopener noreferrer"
              className="text-blue-700"
            >
              {formData.googleClassroomLink}
            </a>
            <div className="space-y-2">
              <Dropdown
                label="Have you joined the Google Classroom?"
                name="joinedGoogleClassroom"
                value={formData.joinedGoogleClassroom}
                onChange={handleChange}
                options={[
                  { label: "Select", value: "" },
